...
2. Bind roles with `link_list`, using full terminal names and verifying cwd/repo.
   ...
   | Reviewer | Independently reviews the actual diff, read-only; must differ from implementer |
...
task's implementation; otherwise the ledger record suffices. **Changed outcomes
or material risk require user ratification**; a worker declaration alone never
authorizes expansion. Outside run-through,
...
At ADVANCE and before each new step dispatch, use `link_list` to assess the
...
percentage. A `?` immediately after successful compaction is fresh context, not a
reason to compact again.
...
Once a worker has begun its step of a task, preserve its context through that
...
step dispatch. If an engaged worker cannot fit the remaining work, escalate
...
step, dispatch/callback, gate and uncovered surfaces, verdict, amendments,
...
| Worker's context dropped mid-task (auto-compaction) | Do not interrupt; in its next dispatch, restate the task's current state; treat its retained conclusions as records, not evidence |
