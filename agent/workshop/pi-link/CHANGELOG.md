...
- **`pi-link <name>` now rejects Pi-managed flags even when passed as the first token, and unknown leading flags also error instead of being treated as session names.** Previously `pi-link --link-name foo` or `pi-link --session path` silently treated the flag as a session name; `pi-link --model gpt-4 foo` similarly silently treated `--model` as the name. Managed-flag validation now also runs on the first token (specific error messages preserved); unknown flags before the name produce a clear "Unknown argument before name" error. Pi flags should be passed after `<name>`.
