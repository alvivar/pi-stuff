...
Since this roadmap was written: **`link_compact` shipped and was validated live** — the first slice of the orchestration line, pulled forward on its own plan. Two **code-review hardening passes** also landed (review refinements + follow-ups: disconnected-result error contract, pre-abort fail-fast, hub duplicate-register guard, `link_send` self-target guard), all live-verified across the link. Maintenance only — no effect on this sequence. Their plan files were removed per the delete-after-done convention, and the upstream REPORT/REQUEST drafts were retired as no longer needed; git history is the archive.
...
**What it unblocks:** the orchestration line — `PLAN-orchestration.md` needs an orchestrator to _see_ worker context before acting on it. That input now exists (and its compact slice has since shipped as `link_compact`).
...
**Decisions made (with gpt):** placement is **README, not `examples/`** (`examples/` isn't in the npm `files` allowlist, so it'd be invisible to npm users) and **not SKILL** (agents already have the mechanics; avoids the cookbook). Fan-out is included as a value line (the headline capability); SKILL's "Parallel batch" keeps the _mechanics_ (busy rule, batching, wait-for-all) — README states the shape, SKILL states how to run it safely. SKILL left untouched.
...
- **`PLAN-orchestration.md`** — now unblocked, and partially shipped: `link_compact` is live. The remaining remote tools (`link_set_model`, `link_set_thinking`) let an orchestrator manage workers. Bigger effort; the ambitious direction.
...
- **Upstream issues (former REPORT/REQUEST drafts)** — retired without filing (owner decision, 2026-06-09). If the `sendMessage(triggerTurn:true)` race resurfaces, the full report lives in git history.
