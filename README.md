...
The global install is optional; the Pi install enables the extension, slash commands, and LLM tools. See the package [README](agent/workshop/pi-link/README.md), [CHANGELOG](agent/workshop/pi-link/CHANGELOG.md), bundled `pi-link-tools` skill, and design documents (`BACKLOG.md`, `REPORT-*.md`).
...
It is a policy skill built on `pi-link-tools`, the tool-use skill bundled with `pi-link`.
